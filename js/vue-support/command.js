...
Vue.component("command-input", {
    ...
    data: function () {
        return {
            "matching": "",
            "selected": { "mnemonic": "NON_SELECTED", "args": [] },
            "active": false,
            "cmd_hist_filter": null,
            "hist_table_header": [
                { "text": "Command Time", "value": "str_time" },
                { "text": "Command Id", "value": "hex_id" },
                { "text": "Command Mnemonic", "value": "template.full_name" },
                { "text": "Command Args", "value": "arg_vals" },
            ]
        }
    },
    ...
    methods: {
        ...
        sendCommand() {
            ...
            this.loader.load("/commands/" + command.full_name, "PUT",
                ...
                .then(function () {
                    _self.active = false;
                    _self.send_command_btn_text = "Send CMD"
...
export let CommandMixins = {
    ...
    updateCommandHistory(cmdhist) {
        for (const obj of cmdhist) {
            const str_time = timeToString(obj.time);
            const hex_id = "0x" + obj.id.toString(16);
            const unique_key = "cmd-" + obj.id + "-" + obj.time.seconds + "-" + obj.time.microseconds;
            let new_obj = { ...obj, "str_time": str_time, "hex_id": hex_id, "unique_key": unique_key };
            this.vue.cmdhist.push(new_obj);
        }
    }
};
/**
